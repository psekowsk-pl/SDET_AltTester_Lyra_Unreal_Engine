...
This repository contains AltTester + C# + NUnit automated tests developed for Lyra Starter Game in Unreal Engine 5.3.2.
...
## 1. Setup
Follow these instructions to properly set the Lyra and Test Automation projects.
...
## 2. Troubleshooting
The Unreal Engine 5.3.2 and the Lyra project can have a couple of issues that you need to resolve. Here's a quick instruction on how to deal with them:
...
Here's a piece of code, that you copy and paste:
```
r.RayTracing=False
...
This project depends on the driver, so it can't become stale at any point. Due to the nature of AltTester and runtime dependency on the game state, the project does not use the Page Object Model (POM). Instead, a static utility-based architecture was implemented (static classes, functions and variables). This decision allowed me to create separate files with locators and methods, that I can use any time in the test.

<img width="1038" height="482" alt="obraz" src="https://github.com/user-attachments/assets/dd675789-9564-4499-8e8d-4da0ff803905" />

### b) Utilities
The project is using JSON files and functions for driver configuration. This design allows me to hide critical data (api key, passwords) in secrets.json, so it will be safe from unintentional git push into remote repository.
...
* GetElementByPath() - this method will wait for an object and return it. You need to use the PATH to make it work.
* DoubleCheckClick() - this method will try to click the element multiple times, until the expected object appears. It will decrease the amount of false negative results.
...
These smoke tests were designed to be light and quick, so anyone running them can get quick feedback for core gameplay functionality. This is a good practice, before executing more complex tests.
...
My first idea was to take an inspiration from a different functionality, that we can see in a lot of TPP games, which is a camera movement on the obstacle, behind the player. This one is using an invisible line, so the camera can move closer or further from the player, whenever it's needed. 

I've decided to use something similar, using just 2 coordinates: player's location and target's location. Based on those 2 variables, I can use the built-in UKismetMathLibrary with FindLookAtRotation static function, that can return the direction angle. I had to add a small offset, so the player will have a crosshair directly on the target. To finish the function I have to update the Player's Controller rotation and it is done.

The Unreal Engine 5 is returning the rotation values in string, so I had to create a Parse function, that is splitting them into the more handy struct with public variables.
